density_DifEnd <- function(DifEct){
  ...
  DifEct %>%
      ...
      dim(dfect)
                           pe1<-ggplot(dfect,aes (GE))+
                             ...
                             scale_fill_manual(values=c("green", "blue", "magenta","yellow"))+
                             #ggtitle("Gene expression distribution each layer")+
                             #ylim(0, 0.3)+
                             theme_minimal(base_size = 14)+
                             theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
                             xlab("Gene expression, upEnd")+
                             ylab("density")
                             #annotate(geom = "text", x = 0, y =0.15, label = paste(param2))
                           pe1
}