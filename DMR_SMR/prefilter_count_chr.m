function [winn,level_BEFneg,covEFneg,BE_npos_meNeg,BE_npos_unmeNeg,fracZ_lowE,level_BEF,covEF,BE_npos_meF,BE_npos_unmeF,winEF]=prefilter_count_chr(thr_low,file_win,file_count)
%filter out low coverage windows for CG methylation
display('%------------------count prefiltering'); 

   % win=load(fullfile(winFolder, FilenameWin));
    %Y_muEc = load(fullfile(my_folder, FilenameEct));
     win=load(file_win);
     Y_muEc = load(file_count);
...
%0	4	0	4
    ...
    [winn,level_BEFneg,covEFneg,BE_npos_meNeg, BE_npos_unmeNeg,fracZ_lowE,level_BEF,covEF,BE_npos_meF,BE_npos_unmeF,winEF]=prefilter_internal_May21(thr_low,win,Y_muEc);

end

%-------------------subfunction internal prefiltering
function [winn,level_BEFneg,covEFneg,BE_npos_meNeg, BE_npos_unmeNeg,fracZ_lowE,level_BEF,covEF,BE_npos_meF,BE_npos_unmeF,winEF]=prefilter_internal_May21(thr_low,win,Y_muEc)
